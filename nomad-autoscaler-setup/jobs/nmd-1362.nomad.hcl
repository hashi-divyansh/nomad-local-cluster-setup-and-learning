job "hello-world" {
  ...
  policy {
    check "max" {
      ...
      #query = "avg(avg_over_time(nomad_client_allocs_cpu_total_percent{exported_job=\"hello-world\",task=\"web\"}[5m]))"
      query = "vector(0)"
...
scaling "mem" {
  ...
  policy {
    check "max" {
      ...
      #query = "avg(avg_over_time(nomad_client_allocs_memory_rss{exported_job=\"hello-world\",task=\"web\"}[5m]))"
      query = "vector(0)"
      strategy "app-sizing-max" {}
    }
  }
